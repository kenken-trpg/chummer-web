...
毎回ビルドするのは、`compose.yaml` とイメージがずれないようにするためです。ここを「手元にイメージが
あれば使う」にしていたところ、古いイメージがそのまま使われ、コンテナが `caddy: EPERM` で起動しなく
なる事故がありました（コンテナの権限剥奪と、それに対応する Dockerfile 側の変更はセットで入っています）。
...
- [`docs/deploy.md`](docs/deploy.md) — Docker、Cloud Run / Fly / Cloudflare Containers / 自宅 + Cloudflare Tunnel
