...
#### 5-7. コデックス由来の 3 冊への一般化 ✅ (2026-09-19)

RG の仕組みは 1 冊専用に書いてあったが、**日本語版が存在しない 3 冊** (RF / SG / DT) も
同じ経路に載る。日本語で読める資料が『シャドウラン・コデックス』だからで、これは
「その 1 冊の翻訳」ではなく **3 冊の内容を再録した訳語源**である (フェーズ 5 の冒頭で
コデックスを「原書の追加ではない」と整理したのと同じ理由)。

- 新設 `backend/scripts/ja_books.py` — 登録表。`code` (カタログの `<source>`) /
  `ja_source` (人が何を見て答えるか) / `slug` / `table` / `buckets` (作業順) と、
  **`page_is_ja`**: カタログのページ番号がその資料でも通用するか。RG は真 (5-2 で検証済み)、
  コデックス由来の 3 冊は**偽** — ページは英語版のものなので、台帳を上から読むのではなく
  コデックスの索引で引く。台帳のヘッダにその旨を 1 行出す。
- `make_rg_worksheet.py` → **`make_ja_worksheet.py --book RG`**、
  `import_rg_worksheet.py` → **`import_ja_worksheet.py --book RG`** (既定は `RG` のまま)。
  台帳のファイル名に slug が入るので、同じフォルダに 2 冊分の台帳を置ける。
- `test_rg_coverage.py` → **`test_book_coverage.py`**。台帳の健全性 4 件を全書籍で
  parametrize し、`DECIDED_FLOOR` を書籍ごとの dict に。未着手の本は 0。
- `import_ja_from_refs.py` は登録表を回して各書籍の表を取り込む (モジュールが無い本は
  未着手と同義でエラーではない)。`regen_ja.sh` の出力は**バイト単位で不変**。
- **`=` の効き方が RG と逆である**という実測 (既に日本語で表示されている件数):
  RG 488/506、SG 334/465、RF 91/732、DT 27/188。RG は「突き合わせ」だったが、
  RF と DT は 7〜8 割が**打ち込み**になる。工数はページ数ではなくこの数字で見積もる。
...
- 2026-09-19: フェーズ 5 の仕組みを書籍非依存に一般化。`ja_books.py` (登録表) ＋
  `make_ja_worksheet.py` / `import_ja_worksheet.py` (`--book`) ＋ 書籍ごとの
  `DECIDED_FLOOR` を持つ `test_book_coverage.py`。RF / SG / DT を『シャドウラン・
  コデックス』由来として登録し、空の `ja_curated_{rf,sg,dt}.py` を置いた。
  `data.json` は不変。**3 冊の読み込み作業は未着手**で、実測では RF 641/732・
  DT 161/188・SG 131/465 が「上流訳すら無い」= 打ち込みになる。
