...
`app/models/`, once by hand in `frontend/lib/types/` — and the two drifted
...
def _comments() -> dict[tuple[str, str], list[str]]:
    ...
    for path in sorted(Path(models.__file__).parent.glob("*.py")):
        _file_comments(path.read_text(encoding="utf-8").split("\n"), out)
    return out


def _file_comments(source: list[str], out: dict[tuple[str, str], list[str]]) -> None:
...
def render() -> str:
    ...
    for name in models.__all__:
        ...
        if not isinstance(cls, type) or not issubclass(cls, BaseModel) or name in SKIP:
            continue
