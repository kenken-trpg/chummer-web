...
def test_client_ip_ignores_x_forwarded_for_by_default(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("app.api.deploy._TRUSTED_PROXY_HOPS", 0)
...
def test_client_ip_reads_nth_hop_from_the_right(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr("app.api.deploy._TRUSTED_PROXY_HOPS", 2)
...
def test_client_ip_ignores_cf_connecting_ip_unless_the_deploy_is_behind_cloudflare(
    ...
    monkeypatch.setattr("app.api.deploy._TRUSTED_PROXY_HOPS", 0)
    monkeypatch.setattr("app.api.deploy._TRUST_CLOUDFLARE_IP", False)
    req = _StubRequest({"cf-connecting-ip": "198.51.100.9"})
    assert _client_ip(req) == "10.0.0.1"  # type: ignore[arg-type]

    monkeypatch.setattr("app.api.deploy._TRUST_CLOUDFLARE_IP", True)
...
def test_a_missing_catalog_does_not_hand_out_the_server_path(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    # is about
    monkeypatch.setattr("app.api.catalog._cached_catalog", _boom)
...
def test_validation_error_does_not_echo_the_input() -> None:
    ...
    not sent back, and a deeply nested one is refused, not a 500."""
    big = "x" * 100_000
