...
- **chum5 を書き出すと、読み込み直したときに変わる点を知らせるようにした。**
  ダウンロードした後、書き出したファイルをこのアプリで読み込み直し、武器や資質などが減る・増える、残りカルマやニューヨンが変わるといった差があれば一覧で表示します。
  Chummer.exe 側の解釈の違いまでは分かりませんが、書き出しで取りこぼす項目には気付けます。
...
- **カスタムデータで足した呪文・アデプトパワー・メンタースピリット・複合フォーム・メタマジックなどが、一覧に出なかった問題を直した。**
  魔法まわりの読み込みだけがデータファイルを直接読んでいて、カスタムデータの追加や修正を通っていませんでした。
  ほかの読み込みと同じ入口に揃え、ruff の XML 読み込み禁止（S314）の例外も外して、同じ抜け道が再びできないようにしました。
...
- **設定ファイルの `<compensateskillgroupkarmadifference>` を読むようになった。**
  有効にすると、作成中に技能グループの技能をグループではなく 1 つずつカルマで上げたとき、全員がそろっているレベルをグループの値段に置き換えます（Chummer の `Skill.RangeCost`）。
  計算式は Chummer のものをそのまま使います（例: 火器の 3 技能をそれぞれ 2 まで上げると 2 カルマ安くなる）。グループ自体にレーティングがあるときは対象外です。
  キャリアでのグループの組み直し（`<allowskillregrouping>`）は、上げた順番の履歴がないと再現できないので、これまでどおり変更されていれば未対応の警告に出ます。
- **Improved Ability のレベルの上限を Chummer に合わせた。**
  これまでは MAG ÷ 2 でしたが、Chummer と同じく「選んだ技能の素のレーティング ÷ 2（切り上げ）」、ただし MAG までにしました。技能を選ぶ前は MAG だけが上限です。
  設定ファイルの `<increasedimprovedabilitymodifier>` を有効にすると、上限は「レーティング + レーティング ÷ 2（切り上げ）」になります。
...
- ライフスタイルの無料グリッド購読を、Hard Targets が有効か設定ファイルの `allowfreegrids` がオンのときだけ付けるようにしました（Chummer と同じ）。`ignorecomplexformlimit` と `allowtechnomancerschooling` は Chummer でも使われていないため、未対応扱いにしないようにしました。
...
- **キャラ作成時に持っている精霊・スプライトに、サービス 1 回あたり `<karmaspirit>`（既定 1）カルマを払うようにした。**
  Chummer と同じで、キャリアで新たに縛った分はカルマがかかりません。カルマの内訳に「精霊・スプライトのサービス」の行が加わります。
...
- **同じ RC グループの反動補正アクセサリは、一番高いものだけ数えるようにした。**
  Chummer は `<restrictrecoil>`（どのプリセットでも有効）のもとで、同じ `<rcgroup>` に入るアクセサリ
  （二脚・三脚・ジャイロマウント・フォアグリップなど）の RC を足さずに、一番高いものだけを数える。
  この app はこれまで全部を足していた。設定で `<restrictrecoil>` を無効にすると、今までどおり全部を足す。
- **設定ファイルの容量チェックとニューエンの上限の設定を読むようになった。**
  `<enforcecapacity>` を無効にすると、防具・装備・サイバーウェアの容量超過がエラーにならない。
  `<unrestrictednuyen>` を有効にすると、作成時にニューエンに回すカルマの上限がなくなる。
  `<licenserestricted>` は Chummer でも偽造免許の対象を選ぶリストが変わるだけなので、読んだものとして扱う。
...
- **書き出しの測定器が、Chummer が一つだけ持つものの中も見るようになった。**
  ...
  不具合を見逃していました）。Chummer が一つだけ持ち、項目ごとに読み込むもの
  —— 伝統とメンター精霊 —— の中も比べます。一覧（装備・資質・ヴィークル）は
  これまでどおり見ません。この app が自前のカタログから組み直すもので、
  違って当然だからです。あわせて `make reconcile` から毎回走らせるようにし、
  失敗するのは「両方が書いていて食い違う」場合だけにしました —— 書き落として
  いる項目は、埋めていく先を数えたものであって合否ではないからです。
...
- **書き出した `.chum5` が、Chummer.exe で能力値マイナスにならなくなった（1/2）。**
  ...
  `startingnuyen` を読み戻す）。この app はそれを 1 つも書いていなかったので、exe で開くと
  **払った先はあるのに、払う原資が無い**キャラクターになりました —— 能力値と特殊能力値が
  マイナスになる、という報告の正体です。Chummer 自身のテストセーブ 34 件と突き合わせ、
  31 件で数字まで一致します。残り 3 件は、今の `settings.xml` にもう載っていない
  5.202 当時の Prime Runner の数字と、予算超過で 0 と保存されたセーブです。
  セッティングの読み込み失敗（もう 1 つの報告）も、同じリリースで直しています ——
  そちらは**この症状のもう 1 つの原因**でもありました。

- **書き出した `.chum5` で、アデプトがアデプトとして読まれるようになった（2/2）。**
  Chummer は「このキャラクターが何者か」を保存されたフラグから読みます（`magenabled`
  `adept` `magician` `technomancer` `resenabled` `depenabled` `critter`、それに
  秘伝位階・沈降位階、種族の移動レート、`cfplimit` など）。この app はどれも書いて
  いなかったので、**アデプトはマンディンとして開き**、払ったはずの〈魔法〉が認められ
  ませんでした。この app のタブと同じ根拠（`enabled_tabs`）から書くので、2 つの読み手が
  食い違うことはありません。Chummer 自身のテストセーブ 34 件のうち 33 件で一致します
  （残る 1 件はシェイプシフターの `critter` で、この app がまだメタタイプの
  `<enabletab>` を見ていないため）。あわせて、種族の移動レートはメタタイプのものを
  書くようにしました ―― ミノタウロスはトロールの脚で走ります。

- **書き出した `.chum5` で、セッティングファイルの読み込みに失敗しなくなった。**
  `<settings>` は画面に出る名前ではなく、Chummer が**読み込み済みセッティングを引く
  ときのキー**でした ―― 同梱プリセットなら `<id>` の GUID、自作ならファイル名です。
  この app は表示名（`Standard`）を書いていたので、どちらにも当たらず、Chummer は
  「セッティングファイルを読み込めません」と、**拡張子を落とした名前**を挙げて
  答えます ―― 「同じファイル名はあるはずなのに」の正体です。名前のほうは
  `<gameplayoption>`（5.202 当時の置き場所で、この app の取り込みが最初に読む欄）に
  書きます。あわせて、使用ルールブックを `<sources>` に、カスタムデータの参照先を
  `<customdatadirectorynames>` に書き出すようにしました ―― キーが当たらなかったとき、
  Chummer が「手持ちのどのセッティングで代用するか」を採点する材料です。
  なお、**この失敗はそれ自体が能力値を壊していました**: 代用されたセッティングの
  作成方式が優先度表を使わないものだと、読み込み時に全能力値の `<base>` が 0 に
  されます。

- **メタヴァリアントの種族カルマを、二重に取らなくなった。** 優先度表では
  メタヴァリアントの行が**メタタイプの行を置き換える**もので、足すものではありません。
  「シェイプシフター（狐）」は優先度 C で 5 カルマ、その「人間」ヴァリアントも同じ 5 で、
  合わせて 10 ではありません。この app は両方を足していたので、メタヴァリアントの
  キャラクターは種族に 2 回払わされていました（Chummer 自身のセーブ `Mittens Chargen`
  は 5 と書いています。この app は 10 と言っていました）。
- **偽造 SIN に、偽造免許を何枚でも付けられるようになった。** 装着品の選択肢は
  **すでに付いているものを隠す**ようになっていました。同じ視覚倍率を 2 つ付けても
  意味がないので、その一律の規則は大半の品には正しいのですが、偽造免許は**何の免許か**を
  1 枚ずつ名付けて持つもので、Fake SIN は普通に何枚も抱えます。結果、2 枚目以降が
  選択肢から消えて取得できませんでした。用途を名指しする品（免許、オートソフトの機種、
  スキルソフトの技能）は、隠す対象から外しました。装備・コムリンク・防具の中・
  ヴィークルの車内の 4 か所に同じ規則が書かれていたので、4 か所とも同じ判断に揃えています。

- **装着品の一覧にも、使用ルールブックの設定が効くようになった。** 「改造を追加」「アクセサリを
  追加」のような行ごとの一覧は `<CatalogPicker>` を通らないので、代わりに**ブックの記号が
  コードに直接書かれて**いました（`mod.source === "SR5"`、ときどき `|| "R5"`）。設定から
  一切届かないルールブック一覧です。Ares Predator V に付けられるアクセサリは 29 件から
  120 件に、ビークル改造は 132 件から 151 件になりました。ドラッグの精製度は本ではないので
  常に出ます。

- **カスタムデータが追加した項目が、購入一覧にも出るようになった。** `/api/catalog` は
  カスタムデータを知らず、プロセス寿命でキャッシュした素のカタログを返すだけでした。
  オーバーレイはキャラクターの `compute()` の中でしか掛からないので、**マージしたパックは
  シートには届くのに一覧には届かず**、ハウスルールの武道は「計算できて、印刷できて、
  買えない」状態でした。カタログをデータセット単位で投影し、フロントはデータセットが
  変わったときだけ取り直します。サーバが持っていないデータセットは、キャラクターの
  ときと同じ 409 を返すので、既存の再送がそのまま効きます。
### Added
- **`make reconcile` が、書き出したファイルで何を言い落としているかを言うように
  なった。** `--fidelity` で、Chummer 自身が書いたセーブとこの app の書き出しを
  項目ごとに突き合わせます。今までの `--roundtrip` は**この app 自身の取り込み**で
  読み直すだけなので、「Chummer は要るのにこの app は読まない項目」には構造的に
  気づけませんでした —— どちらも見ていない項目は、どちらも見ていないから無傷で
  戻ってくるのです。Chummer が書いたセーブこそが「Chummer が何を期待するか」の
  唯一の言明なので、そちらと比べます。比べるのはキャラクター自身の頭（誰か、何で
  組んだか、何をいくらまで使えたか）だけです。持ち物の一覧はこの app がカタログから
  組み直すもので、違って当然だからです。テスト用セーブ 34 件すべてで、`sumtoten`・
  `special`・`totalspecial`・`maxkarma`・`gameplayoption` などが落ちていることが
  分かりました（Chummer.exe で読み直すと能力値がマイナスになる、という報告の出どころ）。
- **資質・ウェア・アデプト能力・メンター精霊が連れてくる技能を、技能タブから選べる
  ようになった。** 買うのではなく**名前を決める**だけの技能 —— 資質の技能枠、
  リフレックス・レコーダの対象、アデプト能力〈能力向上〉の対象、メンターの選択が
  連れてくる能力の対象 —— は、それぞれを与えている側（資質、サイバーウェア、アデプトの
  各タブと、メンターの選択欄の中）に選択欄がありました。変えに行く場所としては正しいの
  ですが、**見つける**場所としては誤りで、技能タブには枠があること自体どこにも出て
  いませんでした。知っている人にしか選べない設定です。同じ選択欄を技能タブの先頭にも
  並べ、どれが何から来ているかを併記しました。どちらで選んでも同じものです。

- **スタイル一式を `.zip` でも読み込めるようになった。** Android の Chrome には
  `webkitdirectory` が無く、**フォルダを渡す手段自体が存在しません** ―― スマホや
  Chromebook では、カスタムデータを参照するセッティングがそもそも読み込めませんでした。
  同じフォルダを固めた zip を、通常のファイル選択（「セッティングを読み込む」）から
  渡せるようにしました。展開後のパスはフォルダ選択のときと同じなので、以降の処理は共通です。
  依存は増やしていません（`DecompressionStream` は共有リンクの符号化で既に使っているもの）。
  日本語版 Windows が書く Shift_JIS のファイル名にも対応しています ―― 日本語で書かれた
  スタイルのディレクトリ名は日本語なので、これはこの app では例外ではなく通常のケースです。
- **使用ルールブック外の持ち物を、作成チェックが名指しするようになった。** ブックを外すと
  購入一覧からは消えますが、**すでに持っているもの**は消えません（GM が途中でブックを
  落としても装備を没収したわけではないし、Chummer から取り込んだセーブは来たものが来ます）。
  今まではその手掛かりが「2 つ目が買えない」しか無かったので、品名とブック記号を挙げる
  警告を出すようにしました。止めはしません ―― 卓で確認するためのものです。

### Changed

- **カスタムデータが足りないときの案内に、その場で直す手段を付けた。** セッティング
  ファイル単体を読み込んでもカスタムデータは付いてこない ―― `settings` と `customdata`
  が並んだフォルダを、フォルダごと選ぶ必要があります。今までの薄字 1 行は「無い」とだけ
  言っていて、「別に選ぶものがある」とは言っていなかったので、**この app はハウスルールを
  読めない**と読めてしまっていました。警告色にし、参照している件数を出し、フォルダ選択を
  その行のボタンから開けるようにしました。あわせて、読み込んだ**後**に何が起きるかも
  正確に言うようにしました。
- **購入一覧の既定表示が、使用ルールブックの設定に従うようになった。** 検索欄が空のとき、
  一覧は設定と無関係に SR5 だけを出していました。見えている絞り込みの下に、もう一枚見えない
  絞り込みが敷かれていたことになります ―― 全ルールブックを有効にしても防具は 202 件中 12 件
  しか出ず、ルールブックを**有効にしても**検索するまで何も変わりませんでした。これを外し、
  ルールブック設定だけが既定表示を決めるようにしました（Standard なら今までどおり 12 件、
  Full House なら 172 件中 120 件＋「他 52 件」）。カテゴリのチップも、選ぶと空になる
  ものが出なくなります。ライフスタイルとドラッグの「よく使うものだけ先に出す」一覧は
  ルールではなく利便なので、そのまま残しています。
- **資質とメンターの精霊にも、使用ルールブックの制限が掛かるようになった。** ブック絞り込みは
  `CatalogPicker` / `PickerList` を通る一覧にしか効いていませんでした。資質タブは自前で一覧を
  組んでいたため、Chrome Flesh を外したセッティングでも「プロトタイプ・トランスヒューマン」が
  検索に出て、そのまま取得できました。メンターの一覧も同様です。取得済みの資質は
  「取得済み」節に残り、選択中のメンターはプルダウンに残ります（自分の選択値を黙って落とす
  `<select>` は「メンター無し」に見えるため）。
- **装甲値や値段の行が読めるようになった。** 品名の下の詳細行 ―― `装甲 12 / 1,000¥ /
  入手 2` ―― は、ヒントや脚注と同じ薄いグレーの 0.8rem でした。そこに並んでいるのは
  シートを作っている数値そのものなので、専用の色（`--muted-stat`、背景とのコントラスト
  6.95 → 10.99）に上げ、桁が縦に揃う等幅数字にしました。`.muted` 全体も 0.8rem →
  0.85rem。
- **タブバーが画面の上に留まるようになった。** ギアの一覧を下まで見てから別のセクションに
  移るのに、いちいち一番上まで戻る必要がありました。タブバーを `<header>` の外に出して
  `position: sticky` にしています（sticky は自分の入れ子の中でしか効かないので、ヘッダーの
  中にいる限り留まれませんでした）。狭い画面ではタブを折り返さず横 1 行にします ―― 20 個を
  折り返すと 5 段になり、スマホの画面の 1/4 を常時占有してしまうためです。
- **`make reconcile` が、金額の差がどこにあるかを言うようになった。** `--locate NAME` で、
  セーブ自身が記録している品目の値段と我々の請求額を並べ、「他の費目が全部正しいなら
  この費目はいくらでなければならないか」を出します。全品目がセーブと一致する費目は
  候補から外れるので、たいてい 1 つに絞れます。手作業だった絞り込みがコマンド 1 つに
  なりました。品目の値段がカタログと違うセーブには `gdrift` 印（`qdrift` のギア版）。
- **印刷シートが 2 ページに収まらないときの崩れを直した。** `break-inside: avoid`
  が節ごと（技能表なら 40 行まとめて）に掛かっていました。1 ページより高い塊に
  「割るな」と言っても縮みはせず、残りのページを白紙にしてから結局途中で割れます。
  約束を守れる大きさ ―― **行・リスト項目・ステータス欄** ―― に移し、節は次ページへ
  続いてよいことにして、見出しは `break-after: avoid` で本文に連れて行くようにしました。
  `<thead>` を持つ 5 節では、続きのページで見出し行が繰り返されます。
- **印刷の 2 ページ目にキャラクター名を入れた。** プリンタから出てくるのはばらの紙で、
  今までその 1 枚には誰のシートかを示すものが何もありませんでした。
...
- **`Mundane` の読みを「マンディン」に決めた。** カタログはこの英語名を 2 つの意味で
  ...
  英語のままにしていました。**プレイヤーが見るのは前者だけ**（クリッター能力は
  name/type/action/range/duration で配信され、カテゴリは配信されていない）なので、
  用語集の「マンディン」を採ります。chumJA の「通常生物」だと、作成時に選ぶセルに
  「通常の生物」と出ていました。
