...
import {
  dropDrone, dropTree, vehicleFits, vehicleForbidden, vehicleInteriorFits, wareFitsVehicleMod,
} from "@/lib/character/gear";
import { removeWareTree, wareBounds } from "@/lib/character/ware";

export function VehicleDroneGear({ catalog, character: ch, d, tr, patch, setCharacter, mode }: TabPanelProps & { mode: "drone" | "vehicle" }) {
              ...
              <>
                {((mode === "drone" ? d.drones : d.vehicles) || []).map((item) => {
                  ...
                  <div className="cyber-item" key={item.id}>
                    ...
                    <button className="btn danger" onClick={() => patch(dropDrone(ch, item.id, mode === "vehicle" ? "vehicles" : "drones"))}>削除</button>
                  </div>
                  );
                })}
              </>
                ...
                .map((item) => (
                  <div className="quality-item" key={item.id}>
                    <div>
                      <b>{tr(item.name)}</b>
                      <div className="muted">
                        {item.name} / {tr(item.category)} / HND {item.handling} / SPD {item.speed} / PLT {item.pilot} / SNR {item.sensor} / {item.cost}¥ / {item.avail || "-"} / {item.source}</div>
                    </div>
