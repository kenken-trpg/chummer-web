...
export function LifestyleGear({ catalog, character: ch, d, tr, patch, setCharacter }: TabPanelProps) {
                    ...
                    <div className="cyber-item" key={item.id}>
                      <div>
                        ...
                        <div className="muted">
                          ...
                          {(item.multiplier_pct || 0) !== 0 ? ` / 倍率 ${item.multiplier_pct! > 0 ? "+" : ""}${item.multiplier_pct}%` : ""}
                          {(item.quality_monthly || 0) > 0 ? ` / 品質 +${item.quality_monthly!.toLocaleString()}¥` : ""}
                          {" / "}{item.monthly.toLocaleString()}¥/{lifeIncrement(item.increment)} × {item.months} = {item.nuyen.toLocaleString()}¥
                        ...
                        {(item.qualities || []).map((q) => (
                          <div className="muted" key={q.id} style={{ marginTop: 6 }}>
                            ...
                            {(q.multiplier || 0) !== 0 ? ` / ${q.multiplier! > 0 ? "+" : ""}${q.multiplier}%` : ""}
                            {q.from_freegrid ? " / 付属" : (
                              <>
                                {" "}
                                <button
                        ...
                        {availableQualities.length ? (
                          <div className="cyber-controls">
                            <select
                              ...
                              onChange={(e) => setSlotPick((cur) => ({ ...cur, [qualityPickKey]: e.target.value }))}
                            >
                              <option value="">ライフスタイル品質</option>
                              {availableQualities
                                .filter((q) => {
                                  const s = gearSearch.trim().toLowerCase();
                                  if (!s) return q.source === "SR5" || q.source === "RF" || (q.lp || 0) !== 0;
                                  return q.name.toLowerCase().includes(s) || tr(q.name).toLowerCase().includes(s);
                                })
                                .slice(0, 80)
                                .map((q) => (
                                  <option key={q.id} value={q.id}>
                                    {tr(q.name)} (LP {q.lp}{q.cost ? ` / ${q.cost}¥` : ""}{q.multiplier ? ` / ${q.multiplier}%` : ""})
                                  </option>
                                ))}
                            </select>
                            {(() => {
                              const qspec = availableQualities.find((q) => q.id === slotPick[qualityPickKey]);
                              if (!qspec?.needs_extra) return null;
                              return (
                                <input
                                  placeholder="対象"
                                  value={extraPick[qualityPickKey] || ""}
                                  onChange={(e) => setExtraPick((cur) => ({ ...cur, [qualityPickKey]: e.target.value }))}
                                />
                              );
                            })()}
                            <button
                ...
                .map((item) => (
                  <div className="quality-item" key={item.id}>
                    ...
                    <button className="btn primary" onClick={() => patch({
                      lifestyles: [...(ch.lifestyles || []), { lifestyle_id: item.id, months: 1, quality_ids: [] }],
                    })}>購入</button>
                  </div>
                ))}
      </div>
