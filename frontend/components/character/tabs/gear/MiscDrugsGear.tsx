...
export function MiscDrugsGear({ catalog, character: ch, d, tr, patch, setCharacter, mode }: TabPanelProps & { mode: "misc" | "drugs" }) {
    ...
    <>
              <>
                {(d.gear || []).filter((item) => {
                  ...
                  const drugCat = item.category === "Drugs" || item.category === "Toxins" || item.category === "Chemicals";
                        ...
                        {item.is_drug && item.drug_effect ? (
                          <>
                            <div className="muted" style={{ marginTop: 4 }}>
                              効果: {item.drug_effect}
                              {item.drug_vectors?.length ? ` ／ 経路 ${item.drug_vectors.join("・")}` : ""}
                          ...
                          <div className="cyber-controls">
                            ...
                            >
                              ...
                              {addons
                                ...
                                .filter((mod) => gearSearch.trim() || mod.source === "SR5" || mod.category === "Drug Grades")
                                .map((mod) => (
      ...
      <div className="option-row">
        ...
        {(mode === "drugs"
          ...
          : [...new Set((catalog.gear || []).filter((item) => {
                ...
                .map((item) => (
                  <div className="quality-item" key={item.id}>
                    <div>
                      <b>{tr(item.name)}</b>
                      <div className="muted">
                        {item.name} / {tr(item.category)}{item.maxrating > 0 ? ` / R${item.minrating || 1}-${item.maxrating}` : ""} / {item.cost}¥ / {item.avail || "-"} / {item.source}</div>
                      {item.needs_extra ? (
                        <div className="cyber-controls">
                ...
                .map((item) => (
                  <div className="quality-item" key={item.id}>
                    <div>
                      <b>{tr(item.name)}</b>
                      <div className="muted">
                        {item.name} / {tr(item.category)} / {item.cost}¥ / {item.avail || "-"} / {item.source}
                        {item.effect ? <><br />効果: {item.effect}</> : null}
